...
if __name__ == "__main__":
    ...
    while (i < no_of_positions):
        ...
        if (len(allowed_values) == 1):
            ...
            del test_obj[i]
    #     print("-----\n")
    # print(len(test_obj.keys()))
    # print(len(test_obj.keys()) == len(set(test_obj.keys())))
    # for line in test_obj.values():
    #     print(f"{line[0] + 1}, {line[1] + 1}")
    ...
    for key, (row_i, col_i) in test_obj.items():
        global_full_board[row_i][col_i] = 0
    print_board(global_full_board)
    print(len([x for x in list(chain(*global_full_board)) if x == 0]))
