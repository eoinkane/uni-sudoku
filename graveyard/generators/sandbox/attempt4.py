...
def update_board(row_index, col_index, value):
    full_board[row_index][col_index] = value
    flat_board[(row_index * board_size) + col_index]


def generate_allowed_values(row_index, col_index):
...
def is_valid_update(row_index, col_index, value, **kwargs):
...
if __name__ == "__main__":
    ...
    for i in range(no_of_positions):
        ...
        # print(f"randomed_positions[i] {randomed_positions[i]}")
        # random_position = randomed_positions[i]
        row_index, col_index = get_matrix_references(
            i, board_size
        )
        print(f"row {row_index} col {col_index}")
        allowed_values = generate_allowed_values(row_index, col_index)
        ...
        for number in numbers:
            if (is_valid_update(
                ...
                update_board(row_index, col_index, number)
                break
        print("-----\n")
    print_board(full_board)
